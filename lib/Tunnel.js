...
const debug = require('debug')('sharetunnel:client');
...
module.exports = class Tunnel extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.closed = false;
    if (!this.opts.host) {
      this.opts.host = 'https://sharetunnel.work';
    }
  }
    ...
    (function getUrl() {
      axios
        ...
        .then(res => {
          ...
          if (res.status !== 200) {
            const err = new Error(
              (body && body.message) || 'sharetunnel server returned an error, please try again'
            );
            return cb(err);
          }
  ...
  _establish(info) {
    // increase max event listeners so that sharetunnel consumers don't get
